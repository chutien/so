 ...
 */
  ...
  if (q != SJF_Q) {
    ...
    if (!rdy_head[q]) {		/* add to empty queue */
      ...
      struct proc *ip;
      for (ip = rdy_head[q]; !ip->p_nextready; ip = ip->p_nextready) {
        if (ip->p_nextready->p_expected_time > rp->p_expected_time) {
          rp->p_nextready = ip->p_nextready;
          ip->p_nextready = rp;
          break;
        }
      }
      if (!ip->p_nextready) {					/* add to tail of queue */
        rdy_tail[q]->p_nextready = rp;		/* chain tail of queue */	
        rdy_tail[q] = rp;				/* set new queue tail */
        rp->p_nextready = NULL;		/* mark new end */
      }
